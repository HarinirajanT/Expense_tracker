...
export function computeHealthScore(txs, goals) {
  ...
  const goalProgress = goals.length
    ? goals.reduce((s, g) => s + Math.min(Number(g.current) / Number(g.target), 1), 0) / goals.length
    : 0;
...
export function generateInsights(txs) {
  ...
  const transport = txs.filter((t) => t.type === 'expense' && /uber|ola|transport|fuel|travel/i.test(`${t.source} ${t.description}`));
